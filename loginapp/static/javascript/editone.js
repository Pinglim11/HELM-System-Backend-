//VARIABLES
let oneform = document.querySelectorAll(".one-form");
let container = document.querySelector("#oneform-container");
let addButton = container.querySelector("#oneadd-form");
let removeButton = container.querySelector("#oneremove-form");
let totalForms = container.querySelector("#id_form-TOTAL_FORMS");
let blocker = container.querySelector('oneblocker');


//EVENTS
addButton.addEventListener('click', function() {
     addForm(oneform,blocker,container,totalForms,removeButton);
});
removeButton.addEventListener('click',function() {
     removeForm(".one-form",totalForms,removeButton);
}); 
...
function addForm(allforms,block,contain,managementform,remove,check) {
	...
	while (tempind < selects.length){
		...
		selects[tempind].selectedIndex = -1;
		tempind +=1;
