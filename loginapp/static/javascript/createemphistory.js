...
let totalForms = container.querySelector("#id_4-TOTAL_FORMS");
...
function addForm(allforms,block,contain,managementform,remove,check) {
	...
	let formRegex = RegExp(`id_4-(\\d){1}-`,'g'); //Regex to find all instances of the form number
	let num = Number(managementform.getAttribute("value")); 
	newForm.innerHTML = newForm.innerHTML.replace(formRegex, `id_4-${num}-`); //Update the new form to have the correct form number
	newForm.innerHTML = newForm.innerHTML.replace(RegExp(`4-(\\d){1}-`,'g'), `4-${num}-`); //Update the new form to have the correct form number
